...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if(of)
	{
		...
		//	pi/4 = 4 arctan(1/5) - arctan(1/239)
		std::unique_ptr<MW::INT_T<2*MW_SIZE>> pi_qurter_approximation_ext = std::make_unique<MW::INT_T<2*MW_SIZE>>(*arctan_1_5);

		*pi_qurter_approximation_ext <<= 2;
		*pi_qurter_approximation_ext -= *arctan_1_239;

		//  Round Off
		std::unique_ptr<MW::INT_T<2*MW_SIZE>> r = std::make_unique<MW::INT_T<2*MW_SIZE>>("0x80000000");
		if(MW_SIZE > 1)
			*r <<= (MW_SIZE - 1);

		*pi_qurter_approximation_ext += *r;

		std::unique_ptr<MW::INT_T<MW_SIZE>> pi_qurter_approximation = std::make_unique<MW::INT_T<MW_SIZE>>(MW::split_hi<MW_SIZE>(*pi_qurter_approximation_ext));
		...
		std::unique_ptr<std::pair<MW::INT_T<MW_SIZE>, MW::INT_T<MW_SIZE>>> tmp = std::make_unique<std::pair<MW::INT_T<MW_SIZE>, MW::INT_T<MW_SIZE>>>();
		tmp->first = 0;
