...
int _tmain(void)
{
	std::cout << "Checking for factorability as (2^k)*(3^l)*(5^m)" << std::endl;
	std::cout << "[Version " << __VER << "]" << std::endl << std::endl;

	#if defined __F_235_FAST_MOD__
		std::cout << "Fast modulus implementation" << std::endl;
		...
		std::cout << "STD Library GCD" << std::endl;
	#endif

	#if defined __F_35_DIV_CHK__
		std::cout << "Divisibility Check" << std::endl;
	#elif defined __F_35_DIV_CHK_BOOST__
		std::cout << "Divisibility Check [popcnt]" << std::endl;
	#endif

	#if defined __F_235_FAST_DIV__
		std::cout << "Fast division implementation" << std::endl;
	#endif

	#if !defined __F_235_PRINT__
		std::cout << "Silent Mode. Timing..." << std::endl;
	#endif
	...
	for(DWORD num = BEG; num < BEG + N; num++)
	{
		...
		if(factorizable)
		{
			...
			std::cout << std::dec << std::setw(5) << std::setfill(' ') << num;
			std::cout << "[" << std::hex << std::setw(4) << std::setfill('0') << num << "] ";

			std::cout << std::dec << std::setw(3) << std::setfill(' ');
			std::cout << exp_2 << "," << exp_3 << "," << exp_5;
	...
	std::cout << std::endl;
	std::cout << f_235_num << "/" << N << std::endl;
	std::cout << "T: " << duration.count() << ", microseconds" << std::endl;
	std::cout << std::endl;
