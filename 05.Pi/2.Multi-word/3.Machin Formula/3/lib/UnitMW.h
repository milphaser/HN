...
namespace MW
{
	...
	public:
		...
		const INT_T max(void) const
		{
			std::unique_ptr<INT_T> result = std::make_unique<INT_T>();
			return *result -= 1;
		}
		...
		//  NOTE: Output Carry is ignored
		{
			std::unique_ptr<INT_T> addend = std::make_unique<INT_T>(rhs);
			*this += *addend;
			return *this;
		}
		...
		//	the difference is 2's Complement coded.
		{
			...
			std::unique_ptr<INT_T> subtrahend = std::make_unique<INT_T>();
			//  1.1. 1's Complement
			for(auto i = 0; i < N; i++)
			{
		...
		//	the difference is 2's Complement coded.
		{
			std::unique_ptr<INT_T> subtrahend = std::make_unique<INT_T>(rhs);
			*this -= *subtrahend;
			return *this;
		}
		...
		//  NOTE: High part of the product is truncated
		{
			std::unique_ptr<INT_T> product = std::make_unique<INT_T>(*this);
			UINT carry = 0;     // High Part of the Partial Product
		...
		//  NOTE: High part of the product is truncated
		{
			std::unique_ptr<INT_T> product = std::make_unique<INT_T>();
		...
		{
			std::unique_ptr<DIV_T<N>> result = std::make_unique<MW::DIV_T<N>>();
	...
	{
		std::unique_ptr<INT_T<2*N>> product = std::make_unique<INT_T<2*N>>();
	...
	//  - hi, the high part;
	{
		std::unique_ptr<INT_T<N>> result = std::make_unique<INT_T<N>>();
